...
Every number below is reproducible: `python analysis/explore_data.py` and
`python analysis/score_and_evaluate.py`. Figures measured on the labelled training data are
**in-sample** — the model was fit on those rows and no holdout exists — so they are an upper
bound.
...
thousands they will never get through. Today that is a guess. What this changes is not that
scores exist — it is that the list acquires **an order and a cut line**. That is the product.

**The number to hold onto is calls per conversion.** Working the top tier, a rep closes one
deal for roughly every **four** accounts called. Working the list blind, about **fifteen**.
...
**What the cut costs.** Skipping Tier D means skipping half the list and giving up
**21.8%** of the conversions in it — worth stating unprompted, since a rep will eventually
close a Tier D account and cite it as proof the system is wrong. The answer: those accounts
were never going to be called anyway. This allocates attention that is already insufficient.

**The honest bar.** A free two-fact lookup — *do we hold vendor data, did they start a trial* —
reaches 0.612 AUC with no model and nothing to maintain. The model reaches **0.759**. It earns
its place, and the fallback if it is switched off is known and free.

**The risk worth naming: an information asymmetry, not a data-quality gap.** 40.17% of
accounts have no third-party intent data. That single fact is the strongest predictor in the
dataset — accounts with vendor coverage convert at 8.22% against 3.94% without (p = 0.0047) —
while the *value* the vendor sells carries no detectable signal at all (p = 0.335 across
quartiles). So Cordilla's ranking is substantially driven by **which accounts a third party
has chosen to cover**, under a coverage policy Cordilla cannot observe, verify, or be
notified about when it changes.

The model compounds this: it imputes a missing intent score to 25.3, indistinguishable from a
real 25.3, so uncovered accounts are over-scored by **1.61pp** against their actual 3.94%
rate. Six of the 28 Tier A accounts are in that group, and are flagged individually rather
than absorbed into a clean-looking ranking.

**What I will not supply.** The data contains no revenue, deal size, rep headcount, call
capacity or cycle length. So there is no ROI figure, dollar amount or payback period anywhere
in this submission. Given ACV and rep capacity, calls-per-conversion converts to money in one
step — that input is yours.
...
them, and the monitoring reads that report. With `LANGSMITH_TRACING` set, the
same graph emits the same spans to a hosted backend with no code change. I did *not* adopt its
...
**Guardrails, because shape is not truth.** The structured-output schema guarantees the
response parses; it says nothing about whether a fact was invented. `validate_rationale`
...
run reports to LangSmith, monitors as a separate job. No GPU or low latency needed.
...
only a *floor* on real batch variation — assuming otherwise produced a check that fired on a
0.7% shift.

**The lagging check** (`monitoring/outcome_monitor.py`) tests the actual claim: did Tier A
convert at ~26.7%? It also establishes why it cannot be the alarm. Detecting a fall to 20%
...
where they work, ticket within one working day; any re-baseline is deliberate and recorded,
because re-baselining silently is how the last effort lost credibility. *Outcome broken* —
