...
proof the system is wrong. They were not being called anyway.

**Who these numbers describe.** The labelled cohort is not the untouched population the VP is
pointing at: 59.4% have already been contacted by sales, 51.4% are MQLs, 18.6% started a
trial, and only **5.1% are cold on every signal**. The brief puts cold outreach well
...
driven by **which accounts a third party chose to cover**, under a policy it cannot observe,
verify, or be told about when it changes. The brief's related claim that coverage
skews to larger accounts is directionally true but weak here: 126.5 vs 113.2 employees,
Welch's p = 0.2545.

The model compounds this: a missing intent score is imputed to 25.3, indistinguishable from a
real one, so uncovered accounts are over-scored by **1.61pp** against their actual 3.94%. Six
of the 28 Tier A accounts are in that group, flagged individually.

**What I will not supply.** The data holds no revenue, deal size, headcount or call capacity,
so no ROI figure appears anywhere here. Given ACV and capacity,
...
mocked at the boundary — and a **run report**.
...
plain functions would run this correctly. What holds is that **node boundaries become
observability boundaries** — every node is a span, `run_report.json` is assembled from them,
and the monitoring reads it. `LANGSMITH_TRACING` ships the node spans to a hosted backend
unchanged; the LLM and guardrail sub-spans are my own `Tracer`, one `@traceable` each. Local
by default is deliberate: the run reproduces from a clean clone. I did *not* adopt the cyclic
or LLM-routing machinery: every decision reaching a rep stays deterministic, because where
the premise is lost credibility, *"why is this account on my list?"* needs a stable answer.
...
**11/11** (`python -m agent.guardrails`); two of those are regressions added after a live
model exposed a bug in the check itself.
...
movement. A vendor shedding 0.8pp per run takes ten weeks to get there — silent throughout.
...
(target 59.83%), Tier A share (10% by construction — free drift detection from frozen cutoffs
rather than per-batch percentiles), mean score, and guardrail rejection rate. On an
...
reps use existing prioritisation that day; do not re-run until the input is explained.
...
doing it silently is how the last effort lost credibility. *Outcome broken* — stop presenting
model-ranked tiers and fall back to the two-fact ordering, priced at 0.612 AUC precisely so
that decision need not be made under pressure.
